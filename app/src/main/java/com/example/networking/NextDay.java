...
public class NextDay extends Activity {
    ...
    public void onClick3 (View v) {
        Intent i = new Intent(NextDay.this, FiveDay.class);
        startActivity(i);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.next_day);
        ...
        main_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (user_field.getText().toString().trim().equals(""))
                    ...
                    new NextDay.GetUrlData().execute(url);
                }
            }
        });
    }
    ...
    private class GetUrlData extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                JSONObject day2 = jsonObject.getJSONArray("DailyForecasts").getJSONObject(1);

                double min = day2.getJSONObject("Temperature").getJSONObject("Minimum")
                        .getDouble("Value");
                double max = day2.getJSONObject("Temperature").getJSONObject("Maximum")
                        .getDouble("Value");

                double minR = day2.getJSONObject("RealFeelTemperature").getJSONObject("Minimum")
                        .getDouble("Value");
                double maxR = day2.getJSONObject("RealFeelTemperature").getJSONObject("Maximum")
                        .getDouble("Value");

                String date = jsonObject.getJSONArray("DailyForecasts").getJSONObject(1).getString("Date");
                String[] date1 = date.split("[-:T+]");


                result_info.setText("ЗАВТРА " + date1[2] + "." + date1[1] + "." + date1[0]);
                result_info.append("\n" + city.toUpperCase());
                result_info.append("\n\n\nТемпература " + df.format((max - 32) * 5 / 9) + "\u00B0/" + df.format((min - 32) * 5 / 9) + "\u00B0 (C)");
                result_info.append("\nОщущается как " + df.format((maxR - 32) * 5 / 9) + "\u00B0/" + df.format((minR - 32) * 5 / 9) + "\u00B0 (C)");
                result_info.append("\n\nДнём " + day2.getJSONObject("Day").getString("IconPhrase"));
                result_info.append("\nПорывы ветра " + df.format(day2.getJSONObject("Day").getJSONObject("WindGust").getJSONObject("Speed").getDouble("Value") * 0.4469) + " м/с");
                result_info.append("\n\nНочью " + day2.getJSONObject("Night").getString("IconPhrase"));
                result_info.append("\nПорывы ветра " + df.format(day2.getJSONObject("Day").getJSONObject("WindGust").getJSONObject("Speed").getDouble("Value") * 0.4469) + " м/с");
            } catch (Exception e) {
